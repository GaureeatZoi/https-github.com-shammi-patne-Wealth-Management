...
namespace ZOI.BAL.Services
{
    public class ProductTypeService : IProductTypeService
    {
        ...
        public ProductTypeService(DatabaseContext context, IHttpContextAccessor httpContextAccessor)
        {
            _context = context;
        ...
        public bool IsExsits(string name, long ID)
        {
            ...
            if (_context.ProductType.Where(e => e.Name == name && e.ID != ID).FirstOrDefault() == null)
            {
                IsExsits = false;
            }
        ...
        public bool IsCodeExstis(string Code, long ID)
        {
            bool IsExsits = true;
            //If the condition true the data with this name doestn't have the duplicate value in the database.
            if (_context.ProductType.Where(e => e.Code == Code && e.ID != ID).FirstOrDefault() == null)
            {
                IsExsits = false;
            }
        ...
        public JsonResponse Summary()
        {
            try
            {
                ...
                data = new ADODataFunction().ExecuteDataset(Constants.Procedures.GetProductTypeSummary, null, CommandType.StoredProcedure);
                resp = data.Tables[0].AsEnumerable().Select(a => new JsonResponse
                {
                    Status = a.Field<string>("STATUS"),
                    Message = a.Field<string>("MESSAGE")
                }).FirstOrDefault();
                resp.Data = data.Tables[1].AsEnumerable().Select(a => new ProductTypeMaster
                {
                    ID = a.Field<int>("ID")
                   ,
                    Name = a.Field<string>("ProductName")
                   ,
                    Code = a.Field<string>("ProductCode")
                   ,
                    ProductDescription = a.Field<string>("ProductDescription")
                   ,
                    ProductKYCDescription = a.Field<string>("ProductKYCDescription")
                   ,
        ...
        public ProductTypeMaster GetData(long ID)
        {
            try
            {
                if (ID != 0)
                {
                    return _context.ProductType.AsNoTracking().Where(e => e.ID == ID).FirstOrDefault();
                }
                else
                {
                    return null;
        ...
        public JsonResponse Deactivate(long ID, bool Status)
        {
            try
            {
                ...
                if (ID != 0)
                {
                    ...
                    model.CreatedOn = DateTime.Now;
                    model.CreatedBy = GetUserID();
                    _context.Set<ProductTypeMaster>().Update(model);
                    int i = _context.SaveChanges();
                    if (i != 0)
                    {
                        resp.Status = Constants.ResponseStatus.Success;
