...
namespace ZOI.APP.Controllers
{
    ...
    {
        private readonly IHolidayService _interface;

        JsonResponse resp = new JsonResponse();
        [Obsolete]
        public HolidayController(IHolidayService _IHolidayService, IServiceFactory serviceFactory) : base(serviceFactory)
        {
            _interface = _IHolidayService;
        }
       ...
       // [TypeFilter(typeof(AuthorizeAction), Arguments = new object[] { "Write" })]
        public IActionResult UpdateHoliday(long ID)
        {  // returns Update view
            
            HolidayViewModel model = new HolidayViewModel();
            InitAccessModel(model);
            model.holyday = _interface.GetData(ID);
            if (model != null)
       ...
       // [TypeFilter(typeof(AuthorizeAction), Arguments = new object[] { "Write" })]
        ...
        { // Check uniqueness of Holiday date
