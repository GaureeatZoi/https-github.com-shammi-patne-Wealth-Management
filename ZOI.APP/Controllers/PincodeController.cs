...
namespace ZOI.APP.Controllers
{
    ...
    {
        private readonly IPincodeService _interface;

        JsonResponse resp = new JsonResponse();

        public PincodeController(IPincodeService _IPincodeService, IServiceFactory serviceFactory) : base(serviceFactory)
        {
            _interface = _IPincodeService;
       ...
       // [TypeFilter(typeof(AuthorizeAction), Arguments = new object[] { "Write" })]
        public IActionResult IsExstis(long name, long ID)
       ...
       // [TypeFilter(typeof(AuthorizeAction), Arguments = new object[] { "Read" })]
        ...
        public IActionResult ExportData(int? exportType)
        {
            ...
            var columnHeader = new string[5] { "Pincode", "City", "State", "Country", "IsActive" };
            ...
            else if (exportType == 2)
            {
                ...
                if (list.Count() > 0)
                {
                    ...
                    {
                        var workSheet = package.Workbook.Worksheets.Add("Pincode Master Data");
                        ...
                        {
                            workSheet.Cells[row, 1].Value = list[i].PinCode;
                            workSheet.Cells[row, 2].Value = list[i].CityName;
                           
                            workSheet.Cells[row, 3].Value = list[i].StateName;
                            workSheet.Cells[row,4].Value = list[i].CountryName;
