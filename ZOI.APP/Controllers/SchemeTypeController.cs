...
{
    public class SchemeTypeController : BaseController
    {
        private readonly ISchemeTypeService _interface;
        ...
        [Obsolete]
        public SchemeTypeController(ISchemeTypeService _ISchemeTypeService, IServiceFactory serviceFactory) : base(serviceFactory)
        {
            _interface = _ISchemeTypeService;
        }

       // [TypeFilter(typeof(AuthorizeAction), Arguments = new object[] { "Write" })]
        public IActionResult Index()
        {
            SchemeTypeViewModel model = new SchemeTypeViewModel();
            InitAccessModel(model);
            model.schemeType.ID = 0;
            return View(model);
        }
       ...
       // [TypeFilter(typeof(AuthorizeAction), Arguments = new object[] { "Write" })]
        ...
        {  // returns Update view
            ...
            SchemeTypeViewModel model = new SchemeTypeViewModel();
            InitAccessModel(model);
            model.schemeType = _interface.GetData(ID);
            if (model != null)
            {
       ...
       // [TypeFilter(typeof(AuthorizeAction), Arguments = new object[] { "Write" })]
        ...
        { // Check uniqueness of Scheme Plan 
       ...
       // [TypeFilter(typeof(AuthorizeAction), Arguments = new object[] { "Write" })]
        public IActionResult AddUpdate(SchemeTypeViewModel model)
        {  // Inserts or Udates Data
            if (ModelState.IsValid)
            {
                return Json(_interface.AddUpdate(model.schemeType));
            }
            else
            {
