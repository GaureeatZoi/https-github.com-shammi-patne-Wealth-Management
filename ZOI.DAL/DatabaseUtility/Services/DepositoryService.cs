...
namespace ZOI.BAL.Services
{
    public class DepositoryService : IDepositoryService
    {
        ...
        public DepositoryService(DatabaseContext context, IHttpContextAccessor httpContextAccessor)
        {
            _context = context;
        ...
        public Depository GetData(long ID)
        {  //Used for finding Diository by Id

            if (ID != 0)
            {   try
                {  return _context.Depository.AsNoTracking().Where(e => (e.Id) == ID).FirstOrDefault();
                }
                catch
                {  return null;
                }
        ...
        {  // For Unique Validation of depositoryName
            bool IsExsits = true;
            if (_context.Depository.Where(e => e.DPName == name && e.Id != ID).FirstOrDefault() == null)
            {  IsExsits = false;
            }
            return IsExsits;
        }
        ...
        { if (ID != 0)
            {  resp.Message = "Data not deleted";
                ...
                { var model = GetData(ID);
                    ...
                    _context.Set<Depository>().Update(model);
