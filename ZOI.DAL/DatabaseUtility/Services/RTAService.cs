...
namespace ZOI.BAL.Services
{
    public class RTAService : IRTAService
    {
        ...
        public RTAService(DatabaseContext context, IHttpContextAccessor httpContextAccessor)
        {
            _context = context;
        ...
        public bool IsExsits(string name, long ID)
        {
            ...
            if (_context.RTA.Where(e => e.Name == name && e.Id != ID).FirstOrDefault() == null)
            {
                IsExsits = false;
            }
        ...
        public JsonResponse Summary()
        {
            ...
            {                
                ...
                data = new ADODataFunction().ExecuteDataset(Constants.Procedures.GetRTAsSummary, null, CommandType.StoredProcedure);
                resp = data.Tables[0].AsEnumerable().Select(a => new JsonResponse
                {
                    Status = a.Field<string>("STATUS"),
                    Message = a.Field<string>("MESSAGE")
                }).FirstOrDefault();
                resp.Data = data.Tables[1].AsEnumerable().Select(a => new RTAs
                {
                    Id = a.Field<long>("Id")
                   ,
                    Name = a.Field<string>("RTAName")
                   ,
                    Address = a.Field<string>("RTAAddress")
                   ,
        ...
        public RTAs GetData(long ID)
        {
            try
            {
                if (ID != 0)
                {
                    return _context.RTA.AsNoTracking().Where(e => (e.Id) == ID).FirstOrDefault();
                }
                else
                {
                    return null;
        ...
        public JsonResponse Deactivate(long ID, bool Status)
        {
            try
            {
                ...
                if (ID != 0)
                {
                    ...
                    model.CreatedOn = DateTime.Now;
                    model.CreatedBy = GetUserID();
                    _context.Set<RTAs>().Update(model);
                    int i = _context.SaveChanges();
                    if (i != 0)
                    {
                        resp.Status = Constants.ResponseStatus.Success;
