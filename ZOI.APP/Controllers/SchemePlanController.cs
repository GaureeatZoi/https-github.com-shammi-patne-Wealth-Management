...
{
    public class SchemePlanController : BaseController
    {
        private readonly ISchemePlanService _interface;
        ...
        [Obsolete]
        public SchemePlanController(ISchemePlanService _ISchemePlanService, IServiceFactory serviceFactory): base(serviceFactory)
        {
            _interface = _ISchemePlanService;
        }
       ...
       // [TypeFilter(typeof(AuthorizeAction), Arguments = new object[] { "Write" })]
        ...
        {
            SchemePlanViewModel model = new SchemePlanViewModel();
            InitAccessModel(model);
            model.schemePlan.ID = 0;
            return View(model);
        }
       ...
       // [TypeFilter(typeof(AuthorizeAction), Arguments = new object[] { "Write" })]
        ...
        {  // returns Update view
            ...
            SchemePlanViewModel model = new SchemePlanViewModel();
            InitAccessModel(model);
            model.schemePlan = _interface.GetData(ID);
            if (model != null)
            {
       ...
       // [TypeFilter(typeof(AuthorizeAction), Arguments = new object[] { "Write" })]
        public IActionResult AddUpdate(SchemePlanViewModel model)
        {  // Inserts or Udates Data
            if (ModelState.IsValid)
            {
                return Json(_interface.AddUpdate(model.schemePlan));
            }
            else
            {
