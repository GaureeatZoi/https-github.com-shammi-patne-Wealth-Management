...
{
    public class InvestorCategoryService : IInvestorCategoryService
    {
        ...
        public InvestorCategoryService(DatabaseContext context, IHttpContextAccessor httpContextAccessor)
        {
            _context = context;
            ...
            try
            {
                ...
                if (!IsExsits(model.Name, model.ID))
                {
                    ...
                    if (model.ID == 0)
                    {
                        ...
                        _context.Set<InvestorCategorys>().Add(model);
                    ...
                    else
                    {
                        ...
                        if (models!=null)
                        {
                            ...
                            _context.Set<InvestorCategorys>().Update(models);
                            int i = _context.SaveChanges();
                            if (i != 0)
                            {
                                resp.Status = Constants.ResponseStatus.Success;
        ...
        public bool IsExsits(string name, long ID)
        {
            ...
            if (_context.InvestorCategory.Where(e => e.Name == name && e.ID != ID).FirstOrDefault() == null)
            {
                IsExsits = false;
            }
        ...
        public JsonResponse Summary()
        {
            try
            {
                ...
                data = new ADODataFunction().ExecuteDataset(Constants.Procedures.GetInvestorCategorySummary, null, CommandType.StoredProcedure);
                resp = data.Tables[0].AsEnumerable().Select(a => new JsonResponse
                {
                    Status = a.Field<string>("STATUS"),
                    Message = a.Field<string>("MESSAGE")
                }).FirstOrDefault();
                resp.Data = data.Tables[1].AsEnumerable().Select(a => new InvestorCategorys
                {
                    ID = a.Field<long>("ID")
                   ,
                    Name = a.Field<string>("InvestorCategory")
                   ,
                    Code = a.Field<string>("Code")
                   ,
        ...
        public InvestorCategorys GetData(long ID)
        {
            try
            {
                if (ID != 0)
                {
                    return _context.InvestorCategory.AsNoTracking().Where(e => e.ID == ID).FirstOrDefault();
                }
                else
                {
                    return null;
        ...
        public JsonResponse Deactivate(long ID, bool Status)
        {
            try
            {
                ...
                if (ID != 0)
                {
                    ...
                    _context.Set<InvestorCategorys>().Update(model);
                    int i = _context.SaveChanges();
                    if (i != 0)
                    {
                        resp.Status = Constants.ResponseStatus.Success;
